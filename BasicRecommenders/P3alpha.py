...
from RecKit.check_matrix import check_matrix_base
from RecKit.similarityMatrixTopK import similarityMatrixTopK
...
class P3alpha(SimilarityMatrixRecommender, Recommender):
    """ P3alpha recommender """

    RECOMMENDER_NAME = "P3alpha"

    def __init__(self, URM_train):
        super(P3alpha, self).__init__()

        self.URM_train = check_matrix_base(URM_train, format='csr', dtype=np.float32)
        self.sparse_weights = True


    def __str__(self):
        return "P3alpha(alpha={}, min_rating={}, topk={}, implicit={}, normalize_similarity={}, norm={})".format(self.alpha,
                                                                            self.min_rating, self.topK, self.implicit,
                                                                            self.normalize_similarity, self.norm)

    def fit(self, topK=100, alpha=1., min_rating=0, implicit=False, normalize_similarity=False, norm='l1'):
        ...
        Pui = normalize(self.URM_train, norm=self.norm, axis=1)

        #Piu is the column-normalized, "boolean" urm transposed
        X_bool = self.URM_train.transpose(copy=True)
        X_bool.data = np.ones(X_bool.data.size, np.float32)
        #ATTENTION: axis is still 1 because i transposed before the normalization
        Piu = normalize(X_bool, norm=self.norm, axis=1)
        del(X_bool)
        ...
        if self.normalize_similarity:
            self.W_sparse = normalize(self.W_sparse, norm=self.norm, axis=1)
        ...
        self.pred_urm = self.URM_train.dot(self.W_sparse)
        print("P3Alpha fitting done")

    def get_pred_row(self, user):
        return self.pred_urm[user]