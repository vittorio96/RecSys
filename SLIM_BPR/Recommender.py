...
class Recommender(object):
    ...
    def m_recommend(self, target_ids, nRec=10):
        ...
        for tid in target_ids:
                results.append(self.recommend(tid, exclude_seen=self.exclude_seen,
                                               n=nRec, filterTopPop=self.filterTopPop, filterCustomItems=self.filterCustomItems))
        return results
    
    def s_recommend(self, target_id, nRec=10):     
        return self.recommend(target_id, exclude_seen=self.exclude_seen,
                                               n=nRec, filterTopPop=self.filterTopPop, filterCustomItems=self.filterCustomItems)
