...
cdef class Cosine_Similarity:
    ...
    def __init__(self, URM, TopK = 45, shrinkage = 100):

        super(Cosine_Similarity, self).__init__()

        self.n_items = URM.getNumTracks()
        self.shrinkage = shrinkage

        self.TopK = min(TopK, self.n_items)

        URM_csr = URM.getCSR()
        self.user_to_item_row_ptr = URM_csr.indptr
        self.user_to_item_cols = URM_csr.indices
        self.user_to_item_data = np.array(URM_csr.data, dtype=np.float64)

        URM_csc = URM.getCSC()
        self.item_to_user_rows = URM_csc.indices
        self.item_to_user_col_ptr = URM_csc.indptr
        self.item_to_user_data = np.array(URM_csc.data, dtype=np.float64)

        self.n_items = URM_csc.shape[1]
    ...
    cdef double[:] computeItemSimilarities(self, long item_id_input):
        """
        We are computin similarities for an ITEM BASED COLLABORATIVE FILTERING
        ...
        #Compute the numerator of the similarity array
        ...
        #Apply the Shrink term to the similarity array

        n1 = len(self.getUsersThatRatedItem(item_id_input))

        for i in range(len(result)):

            if result[i] != 0:
                n2 = len(self.getUsersThatRatedItem(i))
                result[i] = result[i] / (n1 + n2 + self.shrinkage)
